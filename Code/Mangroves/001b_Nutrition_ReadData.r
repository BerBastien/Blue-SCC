...
# ## CARDIO Read Global Burden Disease Data (start)
#     cardio <- read.csv(paste0(datadir,"cardiovascular_deaths_gbd\\IHME-GBD_2021_DATA-d872f25c-1.csv"))
#     glimpse(cardio)
#     cardio2 <- cardio %>% mutate(ISO3 = countrycode(location_name, origin="country.name", destination="iso3c")) %>% 
#         rename(deaths_base=val) %>% mutate(cause = "cardiovascular") %>% dplyr::select(deaths_base,ISO3,cause) %>% 
#         left_join(ssp_gdp_pop_nut %>% dplyr::filter(year==2021,ssp=="SSP2",nutrient=="Omega3"),by="ISO3") %>% 
#         mutate(deaths_base_percapita = deaths_base / (Pop.million*10^6))  %>% dplyr::select(deaths_base,ISO3,cause,deaths_base_percapita)
#     glimpse(cardio2)

#     dependence = 0.01
#     no_subs = 0.5
#     ssp_gdp_pop_nut_risk <- ssp_gdp_pop_nut  %>% left_join(cardio2,by="ISO3") %>% 
#         mutate(deaths_percap_future = deaths_base_percapita * delta_risk*no_subs*dependence, 
#         scenario=paste0(ssp,substr(rcp, nchar(rcp) - 1, nchar(rcp)))) %>% 
#                 mutate(deaths_future = deaths_percap_future * Pop.million*10^6, 
#                 GDPpc_2020USD = (GDP.billion2005USDperYear*(10^9)*def_mult$def_2005_to_2020) / (Pop.million*10^6)) 
#     glimpse(ssp_gdp_pop_nut_risk)

#     write.csv(ssp_gdp_pop_nut_risk,"Data/Modules/fish/Nutrition/ssp_gdp_pop_nut_risk.csv")

   



# ## CARDIO Read Global Burden Disease Data (end)
...
## ALL CAUSES Read Global Burden Disease Data (start)
    ...
    # ggplot(gbd_deaths_number_doseresponse_pop) +
    # geom_point(aes(x=log(GDP.billion2005USDperYear/Pop.million),y=log(deaths_base_percapita),color=income))
    ...
    # ggplot(gbd_deaths_number_doseresponse_pop_temp_future_dep %>% filter(ssp=="SSP2",year==2021)) +
    #     geom_point(aes(x = Nutritional_D, y=-100*future_risk, color=cause, shape=nutrient, group=interaction(cause,nutrient))) +
    #     geom_text_repel(data=gbd_deaths_number_doseresponse_pop_temp_future_dep %>% filter(ssp=="SSP2",year==2021,cause=="Neonatal preterm birth"), 
    #         aes(x = Nutritional_D, y=-100*future_risk, color=cause, label=ISO3))+
    #         theme_minimal()+
    #         ylab("Protective Effect from Fisheries and Mariculture \n(Avoided Diseases as % of 2020 Disease Burden)") +
    #         labs(title="Projections under SSP2-4.6") 
            
    # ggplot(gbd_deaths_number_doseresponse_pop_temp_future_dep %>% filter(ISO3=="MEX",ssp=="SSP2",year>2020)) +
    #     geom_line(aes(x = year, y=lives_saved_percapita_future*10^6, color=nutrient, group=interaction(cause,nutrient))) +
    #     xlim(c(2020,2120))+
    #     geom_text_repel(data=gbd_deaths_number_doseresponse_pop_temp_future_dep %>% filter(ISO3=="MEX",ssp=="SSP2",year==2100),
    #         aes(x = year, y=lives_saved_percapita_future*10^6, color=nutrient, label=cause),size=2.5) +
    #         theme_minimal()+
    #         ylab("Lives Saved per million persons") +
    #         labs(title="Projections under SSP2-4.6") 
            
    # ggplot(gbd_deaths_number_doseresponse_pop_temp_future_dep %>% filter(ISO3=="MEX",ssp=="SSP2",year>2020)) +
    #     geom_line(aes(x = year, y=-100*future_risk, color=nutrient, group=interaction(cause,nutrient))) +
    #     xlim(c(2020,2120))+
    #     geom_text_repel(data=gbd_deaths_number_doseresponse_pop_temp_future_dep %>% filter(ISO3=="MEX",ssp=="SSP2",year==2100),
    #         aes(x = year, y=-100*future_risk, color=nutrient, label=cause),size=2.5) +
    #         theme_minimal()+
    #         ylab("Avoided Health Conditions \n(% of 2020 Levels)") +
    #         labs(title="Projections under SSP2-4.6") 
        
    # ggplot(gbd_deaths_number_doseresponse_pop_temp_future_dep %>% filter(ISO3=="MEX",ssp=="SSP2",year>2020)) +
    #     geom_line(aes(x = year, y=-100*delta_risk, color=nutrient, group=interaction(cause,nutrient))) +
    #     xlim(c(2020,2120))+
    #     geom_text_repel(data=gbd_deaths_number_doseresponse_pop_temp_future_dep %>% filter(ISO3=="MEX",ssp=="SSP2",year==2100),
    #         aes(x = year, y=-100*delta_risk, color=nutrient, label=cause),size=2.5) +
    #         theme_minimal()+
    #         ylab("Change in Nutrient Intake Protective Effect \n (Percentage Points, Relative to 2020)") +
    #         labs(title="Projections under SSP2-4.6") 
    ...
    #         aes(x = year, y=100*delta_risk, color=nutrient, label=cause),size=2.5) +
    #         theme_minimal()+
    #         ylab("Increase in Health Conditions \n(% of 2020 Disease Burden)") +
    #         labs(title="Projections under SSP2-4.6") 


    # ggplot(gbd_deaths_number_doseresponse_pop_temp_future %>% filter(ISO3=="MEX",ssp=="SSP2",year>2020)) +
    # geom_point(aes(x = deaths_percapita_future, y=100*delta_risk, color=nutrient, group=interaction(cause,nutrient))) +
    # #xlim(c(2020,2120))+
    # geom_text_repel(data=gbd_deaths_number_doseresponse_pop_temp_future %>% filter(ISO3=="MEX",ssp=="SSP2",year==2100),
    #     aes(x = deaths_percapita_future, y=100*delta_risk, color=nutrient, label=cause),size=2.5) +
    #     ylab("Increase in Health Conditions (%)") +
    #     labs(title="Projections under SSP2-4.6") +
    ...
    # ggplot(gbd_deaths_number_doseresponse_pop_temp_future %>% filter(ISO3=="MEX",ssp=="SSP2",year>2020)) +
    # geom_line(aes(x = deaths_percapita_future*Pop.million*10^6, y=100*delta_risk, color=nutrient, group=interaction(cause,nutrient))) +
    # geom_point(data=gbd_deaths_number_doseresponse_pop_temp_future %>% filter(ISO3=="MEX",ssp=="SSP2",year==2100),aes(x = deaths_percapita_future*Pop.million*10^6, y=100*delta_risk, color=nutrient, group=interaction(cause,nutrient))) +
    # #xlim(c(2020,2120))+
    # geom_text_repel(data=gbd_deaths_number_doseresponse_pop_temp_future %>% filter(ISO3=="MEX",ssp=="SSP2",year==2100),
    ...
    mod <- felm(health_benefits_percGDP~ tdif | 0 | 0 | 0, data=deaths_by_country_vsl %>% filter(ISO3=="SLB"))
            coef(mod)[1]
    ...
    #HERE
        ...
        #%>% group_by(ISO3,nutrient) %>% 
        summarize(deaths_base_percapita = sum(deaths_base_percapita,na.rm=TRUE))
    
    gbd_deaths_number_doseresponse_pop_agg %>% filter(ISO3=="MEX")
...
## Get Economic Valuation of Daly Change and Damage Function (start)

    VSL = data.frame(ISO3="USA",vsl_million2020USD=10.5) 
    vsl_base_us_million2020USD = 10.05 #based on https://www.nature.com/articles/s41586-022-05224-9
    gdp_base_us_million2020USD = ssp_gdp_pop_nut_risk %>% 
                                filter(year==2020,ISO3=="USA",ssp=="SSP2",nutrient=='Omega3') %>% 
                                dplyr::select(GDPpc_2020USD) 

    ssp_gdp_pop_nut_risk_vsl  <- ssp_gdp_pop_nut_risk %>% 
        mutate(vsl= vsl_base_us_million2020USD * ( GDPpc_2020USD / gdp_base_us_million2020USD[[1]])) %>% 
        mutate(health_damage = vsl * deaths_future, 
                health_damage_percGDP = 100* vsl * deaths_future / (GDP.billion2005USDperYear*10^3*def_mult$def_2005_to_2020))

    
    write.csv(ssp_gdp_pop_nut_risk_vsl,"Data/Modules/fish/Nutrition/ssp_gdp_pop_nut_risk_vsl.csv")

    health_tcoeff <- ssp_gdp_pop_nut_risk_vsl %>% filter(!is.na(health_damage_percGDP)) %>% 
        group_by(ISO3) %>%
        nest() %>%
        mutate(
            health_coeff = map_dbl(data, ~{
        ...
        glimpse(health_tcoeff)
        write.csv(health_tcoeff,"Data/Modules/fish/Nutrition/health_tcoeff.csv")
            


## Get Economic Valuation of Daly Change and Damage Function (end)
