...
def scc_mc(run_type, mc_id):
    if not (root / run_type / f'results/results_ocean_damage_pulse_{mc_id}.gdx').is_file():
        return pd.DataFrame()
    ocean_damage_gdx = gdxpds.read_gdx.to_dataframes(root / f'{run_type}/results/results_ocean_damage_{mc_id}.gdx')
    ocean_damage_pulse_gdx = gdxpds.read_gdx.to_dataframes(root / f'{run_type}/results/results_ocean_damage_pulse_{mc_id}.gdx')
    ocean_today_gdx = gdxpds.read_gdx.to_dataframes(root / f'{run_type}/results/results_ocean_today_{mc_id}.gdx')
    _l = []
