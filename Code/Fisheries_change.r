#setup#
    ...
    dir1 <- 'C:\\Users\\basti\\Documents\\GitHub\\BlueDICE\\Data'
    fish_models <- c("ecoocean","boats")
    #clim_models <- c("hadgem2-es","gfdl-esm4","ipsl-cm5a-lr","miroc5")
    clim_models <- c("gfdl-esm4","ipsl-cm6a-lr")
    clim_scen <- c("ssp585","ssp126")
    sceni <- 1
    climi <- 1
    fishi <- 1
    c <- 1
    ...
    for (sceni in 1:2){
        for(fishi in 1:length(fish_models)){
            for(climi in 1:length(clim_models)){
                ...
                dname <- "tc"
                ncname <- paste(dir1,"\\",fish_models[fishi],"\\",fish_models[fishi],"_",clim_models[climi],"_nobasd_",clim_scen[sceni],"_2015soc_default_tc_global_monthly_2015_2100.nc",sep="")
                ncin <- nc_open(ncname)
                ...
                yearly_totalcatch <- array(numeric(),c(360,180,86)) 
                for (d in (1:(dim(var_array)[3]/12))){ 
                    yearly_array <- var_array[,,(1+(d-1)*12):(12*d)] #decade d of data
                    yearly_sum <- rowSums(yearly_array, dims = 2, na.rm = TRUE)
                    yearly_totalcatch[,,d] <- yearly_sum
                }

            
                time_y <- 2014+ seq(1: ( dim(yearly_totalcatch)[3])) #months since 1601
                #numperiods <- 4
                numperiods <- 8
                ...
                save(periods_brick,file=paste("Data\\periods_",fish_models[fishi],"_brick_",clim_models[climi],"_",clim_scen[sceni],".Rdata",sep=""))
                save(dif_periods_brick,file=paste("Data\\dif_periods_",fish_models[fishi],"_brick_",clim_models[climi],"_",clim_scen[sceni],".Rdata",sep=""))
