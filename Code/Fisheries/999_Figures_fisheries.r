    ...
    ## Deaths by Country (start)
      ...
      ggplot(deaths_by_country %>% filter(ssp=="SSP2",year==2021,ISO3!="LUX")) +
        ...
        ggplot(deaths_by_country %>% filter(ssp=="SSP2",year>2020)) +
          ...
          ggplot(deaths_by_country %>% filter(ssp=="SSP2",year==2100,ISO3!="LUX")) +
              ...
              theme_minimal() +
                  ...
                  scale_color_manual(values=hex_R5)+
                  scale_fill_manual(values=hex_R5) +
                  labs(size="Mortality Rate Baseline", x = "Nutritional Dependency on Fisheries", y = "Avoided Deaths per million persons", title="Avoided Deaths in 2100")


    ## Deaths by Country (end)

    ## Health Damages in Dollars (start)
      deaths_by_country_vsl <- read.csv("Data/Modules/fish/Nutrition/deaths_by_country_vsl.csv")
...
## Figures Nutrition OLD (start)
    
    

#     ## Risk Change (start)
#       ssp_gdp_pop_nut <- read.csv("Data/Modules/fish/Nutrition/nut_riskchange.csv")

#       ggplot(ssp_gdp_pop_nut %>% filter(year >2020, !is.na(rcp)))+
#         geom_point(aes(x=year,y=future_risk,color=rcp))+
#         scale_color_manual(values=hex_ssps)+
#         theme_minimal()+
#         ylab("Relative Risk due to Omega-3 intake")+
#         guides(color=guide_legend("RCP")) 
...
#     ## Deaths increase (start)
      
#       ssp_gdp_pop_nut_risk <- read.csv("Data/Modules/fish/Nutrition/ssp_gdp_pop_nut_risk.csv")

#       # ggplot(ssp_gdp_pop_nut_risk %>% filter(rcp=="RCP60")) +
#       # geom_point(aes(x=year,y=deaths_percap_future*10^6,color=ISO3,shape=scenario))+
#       # ylab("Increase in deaths per million") + 
#       # geom_text(data = ssp_gdp_pop_nut_risk %>% filter(year==2100, rcp=="RCP60"), aes(x=year,y=deaths_percap_future*10^6,label = ISO3)) + 
#       # guides(color="none")+
#       # #scale_y_continuous(trans="log10") +
#       # theme_minimal()

#       glimpse(regions)
#       ssp_gdp_pop_nut_risk <- ssp_gdp_pop_nut_risk %>% mutate(countrycode=ISO3)  %>% left_join(regions, by=("countrycode"))

#       death_rate_2021 <- ggplot(ssp_gdp_pop_nut_risk %>% filter(rcp=="RCP60",year==2021,nutrient=="Omega3")) +
#       geom_point(aes(x=(deaths_base/(10^6)),y=deaths_base_percapita*10^6,color=R5,shape=scenario))+
#       ylab("Cardiovascular deaths per million persons") + 
#       xlab("Deaths (million)")+
#       geom_text_repel( aes(x=(deaths_base/(10^6)),y=deaths_base_percapita*10^6,label = ISO3)) + 
#       guides(color="none",shape="none")+
#       scale_x_continuous(trans="log10") +
#       theme_minimal() +
#       labs(title="Caridovascular deaths in 2021")+
#             scale_color_manual(values=hex_R5)


#       death_projection <-  ggplot(ssp_gdp_pop_nut_risk %>% filter(rcp=="RCP60",year>2021)) +
#       geom_point(aes(x=year,y=deaths_future,color=R5,shape=scenario))+
#       ylab("Additional Deaths") + 
#       geom_text_repel(data = ssp_gdp_pop_nut_risk %>% filter(year==2100, rcp=="RCP60"), aes(x=year,y=deaths_future,label = ISO3)) + 
#       guides(color="none",shape=guide_legend(""))+
#       #scale_y_continuous(trans="log10") +
#       theme_minimal() +
#       labs(title="Additional cardiovascular deaths \ndue to Omega-3 intake decrease")+
#             scale_color_manual(values=hex_R5)
      
#       ggarrange(death_rate_2021,death_projection)
#       ggsave("Figures/SM/fisheries/nut_cardio_omega3_deaths.png")
...
#       ssp_gdp_pop_nut_risk_vsl <- read.csv("Data/Modules/fish/Nutrition/ssp_gdp_pop_nut_risk_vsl.csv")
      
#       ggplot(ssp_gdp_pop_nut_risk_vsl %>% filter(ssp=="SSP2",nutrient=='Omega3',year>2020) )+
#         geom_line(aes(x=year,y=health_damage,group=ISO3)) +
#         ylab("Health Damage (million 2020 USD)") +
#         geom_text_repel(data = ssp_gdp_pop_nut_risk_vsl %>% 
#             filter(ssp=="SSP2",nutrient=='Omega3',year==2100),
#             aes(x=year,y=health_damage,label=ISO3))+
#             theme_minimal()

#       ggplot(ssp_gdp_pop_nut_risk_vsl %>% filter(nutrient=='Omega3',year>2020,ssp!="SSP3") )+
#         geom_point(aes(x=tdif,y=health_damage_percGDP,group=interaction(ISO3,scenario),color=scenario),alpha=0.8) +
#         ylab("Health Damage (million 2020 USD)") +
#         geom_text_repel(data = ssp_gdp_pop_nut_risk_vsl %>% 
#             filter(nutrient=='Omega3',year==2100),
#             aes(x=tdif,y=health_damage_percGDP,label=ISO3))+
#             theme_minimal() + xlab("Global Mean Surface Temperature") +
#             guides(color=guide_legend(""))+
#         scale_color_manual(values=hex_ssps)

#       monetary_dam <- ggplot(ssp_gdp_pop_nut_risk_vsl %>% filter(nutrient=='Omega3',year>2020,ssp!="SSP3") )+
#         geom_point(aes(x=tdif,y=health_damage,group=interaction(ISO3,scenario),color=scenario),alpha=0.8) +
#         ylab("Health Damage \n(million 2020 USD)") +
#         geom_text_repel(data = ssp_gdp_pop_nut_risk_vsl %>% 
#             filter(nutrient=='Omega3',year==2100),
#             aes(x=tdif,y=health_damage,label=ISO3))+
#             theme_minimal() + xlab("Global Mean Surface Temperature") +
#             guides(color=guide_legend(""))+
#         scale_color_manual(values=hex_ssps)
...
#       health_tcoeff <- read.csv("Data/Modules/fish/Nutrition/health_tcoeff.csv")
#       world <- ne_countries(scale = "medium", returnclass = "sf")

#       # Merge your data with the world map data
#       merged_data <- left_join(world, health_tcoeff, by = c("iso_a3" = "ISO3"))


#       quantile_breaks <- quantile(-merged_data$HealthDam_PercentageGDP_perC, probs = seq(0, 1, length.out = 5), na.rm = TRUE)

#       labels2 <- c(
#           paste0("<", round(quantile_breaks[2], 4)),
#           paste0("(", round(quantile_breaks[2], 4), ",", round(quantile_breaks[3], 4), "]"),
#           paste0("(", round(quantile_breaks[3], 4), ",", round(quantile_breaks[4], 4), "]"),
#           paste0(">", round(quantile_breaks[4], 4))
#       )

#       merged_data$damage_category <- cut(
#           -merged_data$HealthDam_PercentageGDP_perC,
#           breaks = quantile_breaks,
#           labels = labels2,
#           include.lowest = TRUE
#       )

#       Dam_coef_map <- ggplot(data = merged_data %>% filter(continent != "Antarctica")) +
#       geom_sf(aes(fill = damage_category )) +
#       scale_fill_manual(values = custom_colors, name =  "Health Damages \n(% GDP-equivalent loss/C)",na.value="transparent") +
#       coord_sf(crs = "+proj=robin") + # Robinson projection
#       theme_minimal() +
#       labs(fill = "Health Damages \n(% GDP-equivalent loss/C)")
      
#       print(Dam_coef_map)
    
#       ggarrange(monetary_dam,Dam_coef_map,ncol=1)
#       ggsave("Figures/SM/fisheries/health_dam_deaths.png")
      
#     ## VSL (end)
# ## Figures Nutrition OLD (end)
