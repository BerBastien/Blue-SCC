...
# Read future coral cover (start)
    ...
    corals_long2$cover_change <- 100*corals_long2$cover_change #Is this the percent change or the change in percentual points?

    
    glimpse(corals_long2)
    
    ggplot(corals_long2)+
    geom_line(aes(x=year,y=cover_change,color=scenario,group=interaction(uniqueplace,scenario)),alpha=0.05)+
    theme_bw()+
    ...
    ggplot(corals_temp)+
    geom_line(aes(x=tdif,y=cover_change,color=scenario,group=interaction(scenario,uniqueplace)), alpha = 0.05)
    ...
    corals_temp_unique <- aggregate(cover_change~uniqueplace+scenario+tdif+year+Latitude.Degrees+Longitude.Degrees,data=corals_temp,FUN="mean")

    ggplot(corals_temp_unique)+
    geom_line(aes(x=tdif,y=cover_change,color=scenario,group=interaction(scenario,uniqueplace)), alpha = 0.05)+
    theme_bw()+
    scale_color_manual(values=c("#0072B2", "#D55E00"))+
    geom_text(aes(x=1,y=1.5,label="RCP4.5 - 2050"),color="#0072B2",size=3) +
    ...
    ylab("Coral cover change from baseline (pp)")+
    geom_text(aes(x=0.2,y=1.5,label="Baseline\n(1997-2018)"),color="black",size=3) # "#0072B2", "#D55E00"
...
# Get temperature regression estimate at each location (start)
    ...
    m_all <- felm(cover_change~0+I(tdif^2)|year+scenario|0|uniqueplace, 
...
## Get hecatres of coral cover at each location (start)
    ...
    #ggsave("Figures/GlobalDistributionCoral.png",dpi=600)
        ...
        corals_t2coeff <- corals_temp_unique_sf_wgs84 %>%
        group_by(uniqueplace) %>%
        nest() %>%
        mutate(
            t2coeff = map_dbl(data, ~{
            mod <- felm(cover_change ~ 0 + I(tdif^2) | 0 | 0 | 0, .x)
            coef(mod)[1]
            })
        )

        min(corals_temp_unique_sf_wgs84$cover_change)
        
        mod <- felm(cover_change ~ 0 + I(tdif^2) |0| 0 | 0,data=corals_temp_unique_sf_wgs84)
        summary(mod)

        # Create a new sf dataframe with the desired columns
        corals_t2coeff_sf <- corals_t2coeff %>%
        left_join(corals_temp_unique_sf_wgs84 %>% distinct(uniqueplace, .keep_all = TRUE)) %>%
        select(uniqueplace, t2coeff, geometry) %>%
        st_as_sf()

        # Check the resulting sf dataframe
        glimpse(corals_t2coeff_sf)
        min(corals_t2coeff_sf$t2coeff)*1.5^2
    ...
    ## Crop BBOX (start)
        ...
        coral_temp_gulf <- st_crop(corals_t2coeff_sf, gulf_bbox)
    ...
    ## Plot (start)
        ...
        summary(coral_temp_gulf$cover_change)

        coral_areas_temp <- ggplot() +
        geom_sf(data = coral_areas_gulf, aes(fill = log10(GIS_AREA_K)), color = NA)+
        ...
        #) +
        geom_sf(data = coral_temp_gulf, aes(color = t2coeff), size = 2) +
        ...
        breaks <- seq(min((coral_temp_gulf$t2coeff)), max((coral_temp_gulf$t2coeff)),length.out=10)

        # Create categorical variable based on GIS_AREA_K values
        coral_temp_gulf$area_group <- cut((coral_temp_gulf$t2coeff), breaks = breaks, labels = FALSE)
        pal1 <- heat.colors(10)
        colors <- colorRampPalette(pal1)(length(breaks) - 1)

        count_temp <- ggplot(coral_temp_gulf[which(!is.na(coral_temp_gulf$area_group)),], aes(x = t2coeff, fill = factor(area_group))) +
        geom_histogram(bins = 50, color = "black") +
        scale_fill_manual(values = colors,guide="none") +
        
        #scale_x_log10(limits=c(min(log10(coral_areas_gulf$GIS_AREA_K)),max(log10(coral_areas_gulf$GIS_AREA_K)))) +
        #scale_x_log10() +
        labs(x = expression(paste("Cover damage (pp/T"^2, ")")), y = "Count", fill = "Cover change") +
        theme_minimal()
        count_temp
        ...
        ggsave("Figures/corals/CoralChange_temp_area.png",dpi=600)
    ## Plot (end)   
    ...
    ## Plot super zoom
        ...
        coral_temp_keys <- st_crop(corals_t2coeff_sf, keys_bbox)
        ...
        #,breaks = c(-10,-5,0,5),labels = c(exp(-10),exp(-5),exp(0),exp(5))
        #) +
        geom_sf(data = coral_temp_keys, aes(color = t2coeff), size = 2) +
        scale_color_gradientn(colors = pal1_heat[1:7], name = expression(paste("Cover damage (pp/T"^2, ")")), guide = "legend") +
        geom_sf(data = continents, fill = "white", color = "gray", alpha = 0.5) +
        ...
        scale_color_gradientn(colors = pal1_heat[1:7], name = expression(paste("Cover damage (pp/T"^2, ")")), guide = "legend") +
        geom_sf(data = continents, fill = "white", color = "gray", alpha = 0.5) +
        labs(title = "Surveys in Coral Areas in Florida Keys") +
        coord_sf(xlim = c(-80.5, -80.2),
                ylim = c(24.9, 25.3)) +
        my_theme()+
        ...
        scale_fill_gradientn(colors = pal1_heat[1:7],name = expression(paste("Cover damage (pp/T"^2, ")")), guide = guide_colorbar(nbin = 50))+#,labels = c("1e-6","1e-4","0.01","1","100")) +
        geom_sf(data = coral_temp_keys,color = "white", size = 2) +
        scale_color_gradientn(colors = pal1_heat[1:7], name = expression(paste("Cover damage (pp/T"^2, ")")), guide = "legend") +
        geom_sf(data = continents, fill = "white", color = "gray", alpha = 0.5) +
        labs(title = "Estimated damage coefficient in Florida Keys") +
        coord_sf(xlim = c(-80.5, -80.2),
                ylim = c(24.9, 25.3)) +
        my_theme()+
        ...
        ggsave("Figures/corals/meandamage.png",dpi=600)
