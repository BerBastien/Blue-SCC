...
# Join with nutritional effect summary and filter for valid entries
gbd_deaths_number_doseresponse_pop <- gbd_deaths_number_pop %>%
    ...
    summarize(
        ...
        #risk_w_avg_tcoeff_var = sum((se * magnitude_risk)^2) / sum(magnitude_risk)^2,
        risk_w_avg_tcoeff_se = sqrt(sum((se * magnitude_risk)^2) / sum(magnitude_risk)^2),
        sum_magnitude_risk = sum(magnitude_risk),
        #sum_magnitude_risk_var = sum(magnitude_risk_var),
...
transmute(effect = risk_w_avg_tcoeff*sum_magnitude_risk)

gbd_deaths_number_pop %>%
    left_join(nut_eff_sum, by = c("condition_gbd", "income")) %>%
    filter(!is.na(nutrient), !is.na(tcoeff)) %>%
    rename(cause = condition_gbd) %>%
    select(deaths_base_percapita, ISO3, cause, nutrient, total_effect, total_se, tcoeff, se) %>% 
    mutate(effect = deaths_base_percapita*total_effect*tcoeff) %>% group_by(ISO3) %>% 
    summarize(effect = sum(effect)) %>% filter(ISO3 %in% c("MEX")) %>% as.data.frame()


# gbd_w <- gbd_deaths_number_doseresponse_pop %>% group_by(nutrient,ISO3) %>% 
# summarize(mortality_w_avg_eff = sum(total_effect*deaths_base_percapita)/sum(deaths_base_percapita),
# mortality_w_avg_se = sqrt(sum((total_se*deaths_base_percapita)^2)/sum(deaths_base_percapita)^2), 
# sum_mortality = sum(deaths_base_percapita,na.rm=T), 
# tcoeff=first(tcoeff),se=first(se)) %>% ungroup() %>% 
# group_by (ISO3) %>% 
# mutate(magnitude_risk = sum_mortality*mortality_w_avg_eff) %>% 
# summarize(magnitude_w_avg_tcoeff = sum(tcoeff*magnitude_risk)/sum(magnitude_risk),
# magnitude_w_avg_tcoeff_se = sqrt(sum((se*magnitude_risk)^2)/sum(magnitude_risk)^2), 
# sum_mortality_w_avg_eff = mean(mortality_w_avg_eff), 
# sum_mortality = sum(sum_mortality)) %>% ungroup()
